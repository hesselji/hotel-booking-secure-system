...
BEGIN
  ...
  -- 1. Cek total stok fisik kamar dari tabel rooms
  SELECT stock INTO v_total_stock
  FROM `rooms`
  WHERE id = p_room_id;

  -- 2. Hitung berapa kamar yang sudah lunas/dipesan pada rentang tanggal tersebut
  ...
  -- 3. Kamar tersedia JIKA jumlah yang dipesan masih di bawah total stok
  SET p_is_available = IF(v_conflict < v_total_stock, 1, 0);
END$$
